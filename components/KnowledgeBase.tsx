  ...
  return (
    <div className={`fixed md:relative inset-y-0 left-0 z-50 w-full max-w-xs md:w-80 bg-white dark:bg-zinc-950 border-r border-slate-200 dark:border-zinc-800 transform transition-transform duration-300 ease-in-out flex flex-col h-full ${isOpen ? 'translate-x-0 shadow-2xl md:shadow-none' : '-translate-x-full md:translate-x-0'}`}>
      <div className="p-4 border-b border-slate-200 dark:border-zinc-800 flex justify-between items-center bg-slate-50/50 dark:bg-zinc-900/50 shrink-0">
        ...
        <div className="flex items-center gap-2"><span className="text-[10px] bg-slate-100 dark:bg-zinc-800 text-slate-500 dark:text-zinc-400 px-2 py-0.5 rounded-md font-bold">{documents.length}</span><button onClick={onClose} className="md:hidden w-8 h-8 flex items-center justify-center rounded-full text-slate-400 hover:bg-slate-100 dark:hover:bg-zinc-800 dark:text-zinc-400 transition-colors"><i className="fa-solid fa-xmark"></i></button></div>
      </div>
      <div className="p-4 flex-1 overflow-y-auto space-y-3">
        ...
        {selectedIds.size > 1 ? (
          <button type="button" onClick={handleCompare} className="w-full py-3 bg-indigo-600 text-white rounded-xl text-[10px] font-bold uppercase tracking-widest shadow-lg shadow-indigo-500/20 animate-in slide-in-from-top-2 flex items-center justify-center gap-2 hover:bg-indigo-700 transition-all active:scale-95"><i className="fa-solid fa-layer-group"></i>Compare {selectedIds.size} Selected</button>
        ) : documents.length > 0 ? (
          <button type="button" onClick={handleSynthesizeAction} className="w-full py-3 bg-slate-100 dark:bg-zinc-800 text-slate-600 dark:text-zinc-300 rounded-xl text-[10px] font-bold uppercase tracking-widest flex items-center justify-center gap-2 hover:bg-indigo-600 hover:text-white dark:hover:bg-indigo-600 dark:hover:text-white transition-all active:scale-95"><i className="fa-solid fa-sparkles"></i>Synthesize All Data</button>
        ) : null}

        {documents.map((doc) => (
          <div key={doc.id} className={`group p-3 bg-white dark:bg-zinc-900/50 border rounded-xl shadow-sm hover:shadow-md transition-all relative overflow-hidden cursor-pointer active:scale-[0.98] ${selectedIds.has(doc.id) ? 'border-indigo-500 ring-1 ring-indigo-500/20' : 'border-slate-200 dark:border-zinc-800'}`} onClick={() => toggleSelect(doc.id)}>
            <div className="flex items-center justify-between">
              ...
              <button type="button" onClick={(e) => { e.stopPropagation(); onRemoveDocument(doc.id); }} className="opacity-100 md:opacity-0 group-hover:opacity-100 p-2 text-slate-400 dark:text-zinc-600 hover:text-red-500 transition-opacity"><i className="fa-solid fa-trash-can text-[10px]"></i></button>
            </div>
          </div>
        ))}
        {documents.length === 0 && (
      ...
      <div className="p-4 bg-slate-50 dark:bg-zinc-950 border-t border-slate-200 dark:border-zinc-800 shrink-0">
        <div className="space-y-4">
          ...
          <div><p className="text-[9px] font-bold text-slate-400 dark:text-zinc-600 uppercase tracking-widest mb-2 ml-1">Action Area</p><div className="grid grid-cols-2 gap-2">
            ...
            <button type="button" onClick={handleClearMemoryAction} className="flex flex-col items-center justify-center p-3 bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-xl hover:border-red-400 dark:hover:border-red-900 hover:bg-red-50 dark:hover:bg-red-900/10 transition-all group border-b-2 border-red-100 dark:border-red-900/20 active:scale-95">
              <i className="fa-solid fa-trash-can text-slate-400 group-hover:text-red-600 transition-colors"></i>
              <span className="text-[9px] font-bold text-slate-600 dark:text-zinc-400 group-hover:text-red-600 dark:group-hover:text-red-400 uppercase tracking-wider text-center">Clear</span>
            </button>
          </div></div>
        </div>
      </div>
